...
const { loadOptions } = require('@babel/core/lib/config/index');
...
const prepareOptions = (options) => {
  ...
  if (options.module.rules && options.module.rules.some(rule => rule.use.some(loaderConfig => loaderConfig.loader === 'babel-loader'))) {
    const babelConfig = loadOptions()
    if (babelConfig) {
      const babelPlugins = babelConfig.plugins
      if (babelPlugins) {
        const moduleResolverPlugin = babelPlugins.find(o => o.key === 'module-resolver')
        if (moduleResolverPlugin && moduleResolverPlugin.options && moduleResolverPlugin.options.alias) {
          const alias = moduleResolverPlugin.options.alias
